...
const NON_STANDARD=new Set(['reactivated','special_market']);
const S={code:P.get('benchmark')||'ACWI_IMI',h:+P.get('h')||3,sort:'beat_rate',dir:-1,base:[],sum:[],path:[],risk:[],riskCache:new Map(),track:[],bench:[],chart:null};
...
function updateRiskHeaders(){
  ...
  if(sh){sh.dataset.s='risk_sharpe';sh.textContent=`Sharpe (${S.h} év)`;sh.title='Lezárt havi hozamokból, 3M DKJ kockázatmentes referenciával. A mutató hiánya önmagában nem zár ki alapot.'}
  if(so){so.dataset.s='risk_sortino';so.textContent=`Sortino (${S.h} év)`;so.title='Lezárt havi hozamokból, 3M DKJ minimum elvárt hozammal. A mutató hiánya önmagában nem zár ki alapot.'}
}
function tabs(){
...
async function loadTrack(){
  try{
    S.track=await A.all('fund_track_record',{select:'fund_id,status,current_segment_start,current_segment_end,current_segment_months,segment_count,largest_gap_months,default_screener_excluded,reason'});
  }catch(e){
    console.warn('A track-record minősítés nem tölthető be; a screener fail-open módban működik.',e);
    S.track=[];
  }
}
function ensureNonStandardToggle(){
  if(E('showNonStandard'))return;
  const base=E('showInactive'),holder=base?.closest('label');
  if(!holder)return;
  const label=document.createElement('label');
  label.className=holder.className||'';
  label.title='Alapértelmezetten elrejtett, nem standard retail esetek: hivatalosan igazolt újraaktiválások és rendkívüli piaci múltú sorozatok. Az adatminőségi review önmagában nem tartozik ide.';
  label.innerHTML='<input type="checkbox" id="showNonStandard"> Speciális / nem standard sorozatok megjelenítése <span class="info">ⓘ</span>';
  holder.insertAdjacentElement('afterend',label);
}
function trackLabel(status){
  if(status==='reactivated')return 'Újraaktivált';
  if(status==='special_market')return 'Speciális piaci múlt';
  return null;
}
function dateMs(value){const t=value?Date.parse(`${String(value).slice(0,10)}T12:00:00Z`):NaN;return Number.isFinite(t)?t:null}
function hasCurrentHorizon(base,rel){
  // The screener should answer a present-tense question. Historical 5Y windows are
  // not enough: the selected horizon must have a valid comparison ending near the
  // fund's latest observation. This also removes young/reactivated series from 3Y/5Y.
  if(!rel||rel.current_fund_return===null||rel.current_fund_return===undefined||!Number.isFinite(+rel.current_fund_return)||!rel.current_end_date)return false;
  const latest=base.last_obs_date||base.obs_date;
  if(!latest)return true;
  const a=dateMs(latest),b=dateMs(rel.current_end_date);
  if(a===null||b===null)return true;
  const lag=(a-b)/86400000;
  return lag>=-2&&lag<=45;
}
function mergedRows(){
  ...
  const tm=new Map(S.track.map(x=>[String(x.fund_id),x]));
  const q=E('search').value.trim().toLowerCase(),minObs=+E('obs').value,showAll=E('showInactive').checked,showNonStandard=E('showNonStandard')?.checked||false;
  let rows=S.base.map(x=>{
    const rel=sm.get(String(x.fund_id)),risk=rm.get(String(x.fund_id)),track=tm.get(String(x.fund_id));
    return {...x,...rel,...pm.get(String(x.fund_id)),
      risk_sharpe:risk?.sharpe_3m_dkj??null,risk_sortino:risk?.sortino_3m_dkj??null,
      track_status:track?.status??'continuous',track_excluded:!!track?.default_screener_excluded,
      track_reason:track?.reason??null,track_segment_start:track?.current_segment_start??null,
      track_segment_months:track?.current_segment_months??null,
      horizon_eligible:hasCurrentHorizon(x,rel)};
  });
  // Current retail universe first. The separate existing inactive toggle can still
  // intentionally expose stale/inactive rows, but the non-standard toggle never does.
  rows=rows.filter(x=>showAll||x.screen_status==='active');
  // Horizon eligibility is independent of Sharpe/Sortino availability.
  rows=rows.filter(x=>x.horizon_eligible);
  // Reactivated + special-market are opt-in. Data-gap review remains a QA matter.
  rows=rows.filter(x=>showNonStandard||!(x.track_excluded||NON_STANDARD.has(x.track_status)));
...
function renderStatusSummary(){
  ...
  const hidden=S.track.filter(x=>x.default_screener_excluded||NON_STANDARD.has(x.status)).length;
  if(hidden)E('statusSummary').insertAdjacentHTML('beforeend',`<span class="statusline" title="Újraaktivált vagy rendkívüli piaci múltú sorozatok; külön kapcsolóval jeleníthetők meg."><b>${hidden}</b> nem standard (alapból rejtve)</span>`);
...
function riskCell(value,label){
  if(value===null||value===undefined||!Number.isFinite(+value))return `<span title="${label}: ezen az időtávon nincs publikálható havi kockázati mutató. Ez önmagában nem screener-kizárási feltétel.">—</span>`;
  return A.num(value);
}
function rowHtml(x){
  const status=x.screen_status||'active',sub=[x.series_name,x.isin,x.manager].filter(Boolean).join(' · '),tl=trackLabel(x.track_status);
  const tb=tl?` <span class="pill" title="${A.esc(x.track_reason||tl)}">${A.esc(tl)}</span>`:'';
  return `<tr class="${status==='active'?'':'inactive-row'}">
  <td><a class="fund" href="fund.html?id=${x.fund_id}&benchmark=${S.code}&h=${S.h}&rh=${S.h}Y">${A.esc(x.fund_name||x.series_name||x.isin)}</a><div class="sub">${A.esc(sub||x.isin||'—')}</div><span class="${A.statusClass(status)}">${A.statusLabel(status)}</span>${tb}</td>
  ...
  <td class="num">${riskCell(x.risk_sharpe,'Sharpe')}</td>
  <td class="num">${riskCell(x.risk_sortino,'Sortino')}</td>
  <td class="num">${x.observations??'—'}</td>
  </tr>`;
}
function render(){
  const r=mergedRows(),label=short[S.code]||S.code;
  E('caption').textContent=`${label} · ${S.h} éves, jelenleg is érvényes összehasonlító időszakok · HUF-ban · havi frissítés`;
  ...
  E('tableBody').innerHTML=r.map(rowHtml).join('')||'<tr><td colspan="14" class="empty">Nincs a kiválasztott időtávon jelenleg összehasonlítható, a szűrésnek megfelelő alap.</td></tr>';
  E('foot').textContent=`${r.length.toLocaleString('hu-HU')} alap · ${label} · ${S.h} éves aktuális tartási idő`;
  scatter(r);
...
function bind(){
  ...
  ['search','manager','category','currency','risk','obs','showInactive','showNonStandard'].forEach(id=>{const el=E(id);if(el)el.addEventListener(id==='search'?'input':'change',render)});
...
try{
  ...
  await loadTrack();ensureNonStandardToggle();renderStatusSummary();tabs();await Promise.all([loadRelative(),loadRisk()]);render();bind();
